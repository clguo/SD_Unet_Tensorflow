  ...
  def _build_model(self):
    ...
    with tf.variable_scope('unit_1_0'):
      x = res_func(x, filters[0], filters[1],
                   ...
                   activate_before_residual[0])#16in，16out,24*24
    for i in six.moves.range(1, self.hps.num_residual_units):
      with tf.variable_scope('unit_1_%d' % i):#5次
        x = res_func(x, filters[1], filters[1], self._stride_arr(1), False)#16in,16out,24*24

    # 第二组
    with tf.variable_scope('unit_2_0'):
      x = res_func(x, filters[1], filters[2],
                   self._stride_arr(strides[1]),
                   activate_before_residual[1])#16in,32out24*24
    for i in six.moves.range(1, self.hps.num_residual_units):
      with tf.variable_scope('unit_2_%d' % i):
        x = res_func(x, filters[2], filters[2], self._stride_arr(1), False)#32in,32out24*24
        
    # 第三组
    with tf.variable_scope('unit_3_0'):
      x = res_func(x, filters[2], filters[3], self._stride_arr(strides[2]),
                   activate_before_residual[2])#32in,64out24*24
    for i in six.moves.range(1, self.hps.num_residual_units):
      with tf.variable_scope('unit_3_%d' % i):
        x = res_func(x, filters[3], filters[3], self._stride_arr(1), False)#64in,64out24*24
  ...
  def _decay(self):
    ...
    for var in tf.trainable_variables():
      ...
      if var.op.name.find(r'DW') > 0:
        costs.append(tf.nn.l2_loss(var))  #sum(var**2)/2
    # 加和，并乘以衰减因子
    return tf.multiply(self.hps.weight_decay_rate, tf.add_n(costs))

  # 2D卷积
  def _conv(self, name, x, filter_size, in_kernels, out_kernels, strides):
    with tf.variable_scope(name):
      n = filter_size * filter_size * out_kernels#n个参数
      # 获取或新建卷积核，正态随机初始化
      kernel = tf.get_variable(
              'DW', 
              [filter_size, filter_size, in_kernels, out_kernels],
              tf.float32, 
