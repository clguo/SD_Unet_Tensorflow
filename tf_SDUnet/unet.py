# tf_SDUnet is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
# 
# tf_SDUnet is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
# 
# You should have received a copy of the GNU General Public License
# along with tf_SDUnet.  If not, see <http://www.gnu.org/licenses/>.
...
from tf_SDUnet.layers import (weight_variable, weight_variable_devonc, bias_variable,
...
def create_conv_net(x, keep_prob, block_size, channels, n_class, layers=3, features_root=16, filter_size=3, pool_size=2,
                    summaries=True):
    """
    ...
    for layer in range(0, layers):
        with tf.name_scope("down_conv_{}".format(str(layer))):
            ...
            conv1 = conv2d(in_node, w1, b1, keep_prob,block_size)
            tmp_h_conv = tf.nn.relu(conv1)
            conv2 = conv2d(tmp_h_conv, w2, b2, keep_prob,block_size)
    ...
    #------NEW-------
    # w3 = weight_variable([filter_size, filter_size, features, features*2], stddev, name="w3")
    # b3 = bias_variable([features*2], name="b3")
    # in_node1 = conv2d(in_node, w3, b3, keep_prob, block_size,padding='SAME')
    # in_node2 = tf.nn.relu(in_node1)
    # w4 = weight_variable([filter_size, filter_size, features*2, features], stddev, name="w4")
    # b4 = bias_variable([features], name="b4")
    # in_node3 = conv2d(in_node2, w4, b4, keep_prob, block_size, padding='SAME')
    # in_node = tf.nn.relu(in_node3)
    # w5 = weight_variable([filter_size, filter_size, features * 4, features * 2], stddev, name="w5")
    # b5 = bias_variable([features*2], name="b5")
    # in_node5 = conv2d(in_node4, w5, b5, keep_prob, block_size, padding='SAME')
    # in_node6 = tf.nn.relu(in_node5)
    # w6 = weight_variable([filter_size, filter_size, features * 2, features], stddev, name="w6")
    # b6 = bias_variable([features], name="b6")
    # in_node7 = conv2d(in_node6, w6, b6, keep_prob, block_size, padding='SAME')
    # in_node = tf.nn.relu(in_node7)
    ...
    for layer in range(layers - 2, -1, -1):
        with tf.name_scope("up_conv_{}".format(str(layer))):
            ...
            conv1 = conv2d(h_deconv_concat, w1, b1, keep_prob,block_size)
            h_conv = tf.nn.relu(conv1)
            conv2 = conv2d(h_conv, w2, b2, keep_prob,block_size)
    ...
    with tf.name_scope("output_map"):
        ...
        conv = conv2d(in_node, weight, bias, tf.constant(1.0),block_size=tf.constant(1))
...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.keep_prob = tf.placeholder(tf.float32, name="dropblock_probability")
        self.block_size=tf.placeholder(tf.int32,name="dropblock_size")

        logits, self.variables, self.offset = create_conv_net(self.x, self.keep_prob,self.block_size, channels, n_class, **kwargs)
    ...
    def predict(self, model_path, x_test):
        ...
        with tf.Session() as sess:
            ...
            prediction = sess.run(self.predicter, feed_dict={self.x: x_test, self.y: y_dummy, self.keep_prob: 1,self.block_size:1})
...
class Trainer(object):
    ...
    def train(self, data_provider, output_path, training_iters=10, epochs=100, keep_prob=0.9,block_size=7, display_step=1,
    ...
    def store_prediction(self, sess, batch_x, batch_y, name):
        ...
        logging.info("Verification ACC= {:.4f}%, loss= {:.4f}".format(error_rate(prediction,
                                                                                   1-util.crop_to_shape(batch_y,
                                                                                                      prediction.shape)),
                                                                        loss))
