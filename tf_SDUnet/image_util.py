...
class ImageDataProvider(BaseDataProvider):
    ...
    def _load_file(self, path, dtype=np.float32):
        if path.endswith('L_1stHO.png') or path.endswith('R_1stHO.png') or  path.endswith('_manual1.png') or path.endswith('_label.png'):
            return np.array(cv2.imread(path, cv2.IMREAD_GRAYSCALE), dtype=dtype)
        else:
            return np.array(Image.open(path), dtype)

    
    def _load_file_nuclei(self, path, dtype=np.float32):
        # return np.array(Image.open(path), dtype)


        if path.endswith('_binary.tif') or path.endswith('_mask.png') or path.endswith('_label.png'):
            return np.array(cv2.imread(path, cv2.IMREAD_GRAYSCALE),dtype=dtype)
        else:
